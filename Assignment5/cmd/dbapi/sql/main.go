...
var (
	...
	syncPort      = common.GetEnv(common.SyncPortEnv, "60002")
	...
	requestPort   = common.GetEnv(common.RequestPortEnv, "70002")
	clusterKey    = common.GetEnv(common.ClusterKeyEnv, "brdnQn39kyUvA1FtKNV4rVXgEhQOAWyWzsnwJr0SjIA=")
...
func main() {
	...
	if err := initMemberNode(syncHost, syncPort, peerNodeNames[0], peerNodePorts[0], requestPort, clusterKey); err != nil {
		err = fmt.Errorf("exception while initializing Sync Node: %v", err)
		log.Panicf("main: %v\n", err)
	}
